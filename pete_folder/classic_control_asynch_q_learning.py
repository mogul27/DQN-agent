...
class FunctionApprox:
    ...
    def __init__(self, options=None):
        ...
        self.q_hat = self.build_neural_network(self.options.get('adam_learning_rate'))
        self.q_hat_target = self.build_neural_network(self.options.get('adam_learning_rate'))
    ...
    def build_neural_network(self, adam_learning_rate):
        ...
        try:
            ...
            outputs = Dense(len(self.options.get('actions')), activation='linear')(dense_1)
    ...
    def run(self):
        ...
        while True:
            ...
            if len(state_action_buffer) >= self.options.get('async_update_freq'):
                ...
                loss, min_q, max_q = self.process_batch(state_action_buffer)
        ...
        while total_steps < self.options.get('total_steps'):
            ...
            try:
                ...
                print(f"Total steps {total_steps} : Episode {episode_count} took {info['steps']} steps"
                      ...
                      f" : min_q_value = {info['min_q_value']:0.4f} : max_q_value = {info['max_q_value']:0.4f}"
