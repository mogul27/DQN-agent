...
class ActorCriticNetwork(nn.Module):
    ...
    def __init__(self, options):
        ...
        self.actor = nn.Linear(128, len(options.get('actions')))
        # The state value
...
class AsynchActorCritic:
    ...
    def synch_shared_with_local(self):
        ...
        self.log.debug(f"Synched weights local={self.local_actor_critic.checksum()}, "
                       f"shared={self.shared_actor_critic.checksum()}")
...
class AsynchQLearnerWorker(mp.Process):
    ...
    def run(self):
        ...
        self.log.debug(f"Setup asynch actor critic with weights {self.shared_actor_critic.checksum()}")
        ...
        while True:
            ...
            if terminated:
                ...
                experiences.append((np.array(state), action, reward, np.array(next_state),
                                            terminated, action_logit, state_value))

                steps += 1
                if steps > 10000:
                    self.log.info("This episode has run for {steps}, so terminate it")
                    terminated = True
...
class AsynchStatsCollector(mp.Process):
    ...
    def process_controller_messages(self, env):
        ...
        if self.tasks['play'] is not None:
            ...
            self.log.debug(f"Reset weights to {self.policy.actor_critic_network.checksum()}")
    ...
    def plot_rewards(self):
        ...
        plot_filename = self.options.get('plot_filename', 'classic_rewards.png')
...
class AsynchQLearningController:
    ...
    def __init__(self, options=None):
        ...
        self.log.debug(f"Created shared_actor_critic { self.shared_actor_critic.checksum()}")
        self.load_weights()
        self.log.debug(f"Weights loaded shared_actor_critic { self.shared_actor_critic.checksum()}")
    ...
    def train(self):
        ...
        for i in range(self.options.get('num_workers', 1)):
            ...
            self.log.debug(f"give worker shared_actor_critic {self.shared_actor_critic.checksum()}")
        ...
        while episode_count < last_episode:
            ...
            try:
                ...
                if episode_count % self.options.get('stats_every') == 0:
                    ...
                    self.log.debug(f"Sending weights to stats_collector {self.shared_actor_critic.checksum()}")
